...
public class MeanFilterSmoothing
{
	 ...
	 */
	public double[] addSamples(double[] handleData)
	{
	...
	//	filterWindow = (int) (hz * timeConstant);
		filterWindow = 100;
		for (int i = 0; i < handleData.length - filterWindow; i++)
		{
			...
			int j = i;
			while (dataLists.get(i).size() < filterWindow) {
				
				dataLists.get(i).addLast(handleData[j++]);
			}
